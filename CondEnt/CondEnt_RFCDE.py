...
try:
    ...
    GRFForestLocalLinearRegressor = None
...
class multivariate_EWS():
    ...
    def get_ranks(self, res, est_res):
        ...
        n = joint_vals.shape[0]
        pseudo_obs = np.apply_along_axis(lambda col: rankdata(col) / (n + 1), 0, joint_vals)
        ranks = pseudo_obs.T
    ...
    def get_estimated_residuals_LLF(self,
        ...
        if GRFForestLocalLinearRegressor is None:
            raise ImportError("skgrf is required for LLF methods. Install it with: pip install skgrf")
